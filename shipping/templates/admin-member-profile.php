...
<div class="shipping-member-profile-view" dir="rtl">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; background: #fff; padding: 20px; border-radius: 12px; border: 1px solid var(--shipping-border-color); box-shadow: var(--shipping-shadow);">
        <div style="display: flex; align-items: center; gap: 20px;">
            <div style="position: relative;">
                <div id="member-photo-container" style="width: 80px; height: 80px; background: #f0f4f8; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 40px; border: 3px solid var(--shipping-primary-color); overflow: hidden;">
                ...
                <input type="file" id="member-photo-input" style="display:none;" accept="image/*" onchange="shippingUploadMemberPhoto(<?php echo $member->id; ?>)">
            </div>
            <div>
                <h2 style="margin:0; color: var(--shipping-dark-color);"><?php echo esc_html($member->first_name . ' ' . $member->last_name); ?></h2>
            </div>
        </div>
        <div style="display: flex; gap: 10px; align-items: center;">
            <?php if (!$is_member): ?>
                <button onclick="shippingEditMember(JSON.parse(this.dataset.member))" data-member='<?php echo esc_attr(wp_json_encode($member)); ?>' class="shipping-btn" style="background: #3182ce; width: auto;"><span class="dashicons dashicons-edit"></span> تعديل البيانات</button>
            <?php endif; ?>

            <?php if (!$is_subscriber || current_user_can('manage_options')): ?>
                <a href="<?php echo admin_url('admin-ajax.php?action=shipping_print&print_type=id_card&member_id='.$member->id); ?>" target="_blank" class="shipping-btn" style="background: #27ae60; width: auto; text-decoration:none; display:flex; align-items:center; gap:8px;"><span class="dashicons dashicons-id-alt"></span> طباعة الكارنيه</a>
            <?php endif; ?>
            <?php if ($is_sys_manager): ?>
                <button onclick="deleteMember(<?php echo $member->id; ?>, '<?php echo esc_js($member->first_name . ' ' . $member->last_name); ?>')" class="shipping-btn" style="background: #e53e3e; width: auto;"><span class="dashicons dashicons-trash"></span> حذف العميل</button>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="shipping-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 25px; border-bottom: 2px solid #eee; padding-bottom: 10px;">
        ...
        <button class="shipping-tab-btn" onclick="shippingOpenInternalTab('member-chat', this); setTimeout(() => selectConversation(<?php echo $member->id; ?>, '<?php echo esc_js($member->first_name . ' ' . $member->last_name); ?>', <?php echo $member->wp_user_id ?: 0; ?>), 100);"><span class="dashicons dashicons-email"></span> المراسلات والشكاوى</button>
    </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr; gap: 30px;">
            <div style="display: flex; flex-direction: column; gap: 30px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div><label class="shipping-label">الاسم الأول:</label> <div class="shipping-value"><?php echo esc_html($member->first_name); ?></div></div>
                    <div><label class="shipping-label">اسم العائلة:</label> <div class="shipping-value"><?php echo esc_html($member->last_name); ?></div></div>
                    <div><label class="shipping-label">اسم المستخدم:</label> <div class="shipping-value"><?php echo esc_html($member->username); ?></div></div>
                    <div><label class="shipping-label">كود الحساب:</label> <div class="shipping-value"><?php echo esc_html($member->id_number); ?></div></div>
                    <div><label class="shipping-label">رقم الهاتف:</label> <div class="shipping-value"><?php echo esc_html($member->phone); ?></div></div>
                    <div><label class="shipping-label">البريد الإلكتروني:</label> <div class="shipping-value"><?php echo esc_html($member->email); ?></div></div>
                </div>


                <h4 style="margin: 20px 0 10px 0; color: var(--shipping-primary-color);">بيانات السكن والاتصال</h4>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div><label class="shipping-label">المدينة:</label> <div class="shipping-value"><?php echo esc_html($member->residence_city); ?></div></div>
                    <div style="grid-column: span 2;"><label class="shipping-label">العنوان (الشارع):</label> <div class="shipping-value"><?php echo esc_html($member->residence_street); ?></div></div>
                    <?php if ($member->wp_user_id): ?>
                        <?php $temp_pass = get_user_meta($member->wp_user_id, 'shipping_temp_pass', true); if ($temp_pass): ?>
    ...
    <!-- Communication Tab -->
    <div id="member-chat" class="shipping-internal-tab" style="display: none;">
    ...
    <!-- Edit Member Modal -->
    <div id="edit-member-modal" class="shipping-modal-overlay">
        <div class="shipping-modal-content" style="max-width: 900px;">
            <div class="shipping-modal-header"><h3>تعديل بيانات العميل</h3><button class="shipping-modal-close" onclick="document.getElementById('edit-member-modal').style.display='none'">&times;</button></div>
            <form id="edit-member-form" style="padding: 20px;">
                <?php wp_nonce_field('shipping_add_member', 'shipping_nonce'); ?>
...
function shippingUploadMemberPhoto(memberId) {
    const file = document.getElementById('member-photo-input').files[0];
    if (!file) return;

    const formData = new FormData();
    formData.append('action', 'shipping_update_member_photo');
...
function deleteMember(id, name) {
    ...
    const formData = new FormData();
    formData.append('action', 'shipping_delete_member_ajax');
    formData.append('member_id', id);
    formData.append('nonce', '<?php echo wp_create_nonce("shipping_delete_member"); ?>');
...
window.shippingEditMember = function(s) {
...
document.getElementById('edit-member-form').onsubmit = function(e) {
    ...
    formData.append('action', 'shipping_update_member_ajax');
    fetch('<?php echo admin_url('admin-ajax.php'); ?>', { method: 'POST', body: formData })
